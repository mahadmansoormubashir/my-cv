...
const projects = [
  ...
  },
  {
    ...
    color: "#7b2fff",
    icon: "🌾",
    github: "https://github.com/mahadmansoormubashir/harvest-hub", // 👈 Your newly deployed repo!
  },
  {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-purple-500 selection:text-white">
      ...
      <header className="max-w-4xl mx-auto pt-24 pb-16 px-6 text-center md:text-left">
        <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 mb-4">
          Mahad Mansoor Mubashir
        </h1>
        <p className="text-lg md:text-xl text-slate-400 font-medium mb-2">
          Computer Science Student · BSCS 4B
        </p>
        <p className="text-sm text-slate-500">
          Passionate about building low-level architectures, smart systems, and intelligent applications.
        </p>
      </header>
                    ...
                    <div className="mt-5 pt-4 border-t border-slate-800 flex flex-wrap gap-2 items-center justify-between animate-fadeIn">
                      <div className="flex gap-1.5 flex-wrap">
                        {project.tech.map((t, i) => (
                          <span key={i} className="text-xs bg-slate-950 text-slate-400 px-2.5 py-1 rounded-full border border-slate-800 font-medium">
                            {t}
                          </span>
                        ))}
                      </div>
                      ...
                      {project.github && (
                        <a 
                          ...
                          onClick={(e) => e.stopPropagation()} // Keeps card from closing when clicked
        ...
        <section>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {skillCategories.map((category, idx) => (
              <div key={idx} className="bg-slate-900/30 border border-slate-800/80 rounded-xl p-5">
                <h3 className="font-bold text-sm uppercase tracking-wider text-slate-400 mb-4">
                  {category.title}
                </h3>
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill, i) => (
                    <span key={i} className="text-xs bg-slate-900 text-slate-300 px-2.5 py-1.5 rounded-md border border-slate-800/60 font-mono">
                      {skill}
                    </span>
                  ))}
                </div>
