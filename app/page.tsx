...
const projects = [
  {
    ...
    description: "description: An automated intelligence pipeline that collects posts from Reddit and Mastodon every 30 minutes, structures each item using a locally-run LLaMA 3.2 model (zero cloud API calls, zero cost), and serves the data across four surfaces: a searchable /feed, curated /digest, rule-based /alerts, and a /trends view showing rising topics ranked by velocity. Features multi-niche profiles (Tech, Cybersecurity, Startups) switchable from the dashboard — each with independent sources, categories, and alert rules. Sends an automated morning email briefing at 8am daily. Built a custom signal-meter dashboard (HTML/CSS/JS) with real-time filtering. Debugged a cross-source deduplication bug and replaced a cloud Gemini dependency with local Ollama inference.,",
    color: "#e8944a",
    icon: "📡",
